...
if __name__ == "__main__":
    read_P21 = pd.read_excel(r"J:\assпірантура\моя тема\Substation 10kV\0_PD_2021History_GMT2.xlsx",sheet_name='Sheet1',usecols=[0, 1])
    read_P22 = pd.read_excel(r"J:\assпірантура\моя тема\Substation 10kV\0_PD_2022History_GMT2.xlsx",sheet_name='Sheet1',usecols=[0, 1])
    read_Q21 = pd.read_excel(r"J:\assпірантура\моя тема\Substation 10kV\0_QD_2021History_GMT2.xlsx",sheet_name='Sheet1',usecols=[0, 1])
    read_Q22 = pd.read_excel(r"J:\assпірантура\моя тема\Substation 10kV\0_QD_2022History_GMT2.xlsx",sheet_name='Sheet1',usecols=[0, 1])
    read_Weather = pd.read_excel(r"J:\assпірантура\моя тема\Substation 10kV\PD_Weather_GMT2_dummy.xlsx",sheet_name='Sheet1')
    read_Daylight = pd.read_excel(r"J:\assпірантура\моя тема\Substation 10kV\0_Sunrise_sunset.xlsx",sheet_name='Sheet1')    
    ...
    for i, lag in enumerate(lags):
        ...
        with pd.ExcelWriter(rf"J:\assпірантура\моя тема\Code\1\Input\PQ.xlsx", engine="openpyxl", mode="a", if_sheet_exists="replace") as writer:
            Data.dropna().to_excel(writer, sheet_name=f"{lag}")        
        splits = rolling_dates(Data.loc[mask].dropna()) 
        with pd.ExcelWriter(rf"J:\assпірантура\моя тема\Code\1\Input\splits.xlsx", engine="openpyxl", mode="a", if_sheet_exists="replace") as writer:
            pd.DataFrame(splits).drop(columns="m").to_excel(writer, sheet_name=f"{lag}")
    
    # Bring the Datetime index back as a column for export
    #PQ = PQ.reset_index(names='Datetime')
    
    #PQ['Datetime'] = PQ['Datetime'].dt.strftime("%Y-%m-%d %H:%M:%S")

    #PQ.to_excel(r"J:\assпірантура\моя тема\Code\1\Input\PQ.xlsx", index=False)
